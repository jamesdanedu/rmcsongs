// lib/supabase.js - Enhanced with better error handling and YouTube view count support
...
export async function addSong(songData) {
  try {
    ...
    const validSongData = {
      ...
      youtube_view_count: songData.youtube_view_count || null, // Include YouTube view count
    ...
    // Ensure YouTube view count is a valid number if provided
    if (validSongData.youtube_view_count !== null) {
      const viewCount = Number(validSongData.youtube_view_count);
      if (isNaN(viewCount) || viewCount < 0) {
        console.warn('Invalid YouTube view count provided:', validSongData.youtube_view_count);
        validSongData.youtube_view_count = null;
      } else {
        // Store as a bigint for large view counts
        validSongData.youtube_view_count = viewCount;
      }
    }
...
export async function addVote(songId, userId, voteType = 'up') {
  ...
  while (retries > 0) {
    try {
      // Try to update existing pending vote
      const { data, error } = await supabaseClient
        .from('votes')
        .update({
          vote_type: voteType,
        ...
        .eq('vote_status', 'pending')
        .select()
        .maybeSingle(); // Use maybeSingle instead of single for more flexibility
      
      if (error) throw error;
      
      if (!data) {
        // Fallback: Create a vote record if none exists
        console.warn('No pending vote found - creating one');
        const { data: newVote, error: insertError } = await supabaseClient
          .from('votes')
          .insert({
            song_id: songId,
            user_id: userId,
      ...
      retries--;
      console.error(`Vote error (${retries} retries left):`, err);
