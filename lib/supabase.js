...
export async function fetchVotesForSong(songId) {
  try {
    ...
    console.log(`Retrieved ${votes.length} votes for song ID: ${songId}`);
    return votes;
...
export async function addVote(songId, userId) {
  try {
    ...
    if (existingVote) {
      console.log('Vote already exists, returning existing vote');
      return existingVote;
    }
    ...
    if (!data) {
      throw new Error('No data returned after inserting vote');
    }
