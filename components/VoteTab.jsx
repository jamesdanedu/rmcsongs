 ...
 */
const VoteTab = ({ songs = [], username = '', userId = '', handleVote }) => {
    ...
    <div className="bg-white p-6 rounded-xl shadow-md border border-indigo-100 transform transition-all duration-300 hover:shadow-lg">
      ...
      {!Array.isArray(songs) || songs.length === 0 ? (
        <div className="text-center py-10 bg-gradient-to-r from-indigo-50 to-blue-50 rounded-lg border border-indigo-100 shadow-inner">
          <Music size={48} className="mx-auto text-indigo-300 mb-3 opacity-70 animate-pulse" />
        ...
        <div className="space-y-4">
          {songs.map(song => {
            ...
            const hasVoted = hasVoters && (song.voters.includes(username) || (userId && song.voters.includes(userId)));
            
            return (
              <div 
                key={song.id || `song-${Math.random()}`} 
                className="border border-indigo-100 rounded-lg bg-gradient-to-r from-indigo-50 to-blue-50 overflow-hidden shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 transform"
              >
                <div className="p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      {song.notes && (
                        <p className="text-sm text-gray-600 mt-1 border-t border-indigo-100 pt-1 italic">{song.notes}</p>
                      )}
                    ...
                    <div className="flex flex-col items-center ml-4">
                      <div className="bg-white text-indigo-700 rounded-full h-8 w-8 flex items-center justify-center font-bold shadow-sm border border-indigo-100">
                        {song.votes || 0}
                      </div>
                      <button 
                        className={`mt-2 rounded-md px-4 py-1 text-sm flex items-center transition-all duration-200 ${
                          hasVoted
                        ...
                        disabled={hasVoted}
                      >
                        <Heart size={14} className={`mr-1 ${hasVoted ? '' : 'animate-pulse'}`} />
                        {hasVoted ? 'Voted' : 'Vote'}
                      </button>
                    </div>
                  </div>
...
export default VoteTab;import React from 'react';
import { Heart, Music, Mic } from 'lucide-react';
